...
/**
 * 概念解析器: 包括概念的加载，内部组织、索引、查询以及概念的相似度计算等. 概念保存到数组中, 没有保存到Map中, 以尽量降低对内存空间的使用.
 * 算法的核心思想请参看论文《汉语词语语义相似度计算研究》 <br/><br/> improvement: <ol>
 * <li>两个义原集合的运算方式，支持均值方式或Fuzzy方式</li> </ol>
 *
...
public abstract class BaseConceptParser implements HownetMeta, WordSimilarity {
    ...
    /**
     * 获取两个概念之间的相似度，如果两个概念的词性都不相同，则直接返回0; 否则，计算对应义元的相似度，并加权求和
     *
     * @param c1 第一个参与运算的概念
     * @param c2 第二个参与运算的概念
     * @return
    ...
    /**
     * 考虑到义原集合中的义原先后关系影响不大，因此计算两个集合的义原相似度的平均值作为这两个义原集合的相似度, 即此处采用的是均值方法： The
     * rank of a value assignment is the average of the weights of the
     * constituent values.
     *
     * @param sememes1
     * @param sememes2
